    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {filteredCustomers.map(c => {
          ...
          return (
            <div key={c.id} className={`bg-white p-10 rounded-[3rem] border transition-all space-y-8 group relative overflow-hidden ${
              isOverLimit 
                ? 'border-rose-500 shadow-2xl shadow-rose-100 ring-2 ring-rose-500/10 animate-in fade-in duration-700' 
              ...
              <div className="flex justify-between items-start relative z-10">
                <div className="cursor-pointer" onClick={() => { setSelectedForHistory(c); setIsHistoryModalOpen(true); }}>
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className={`font-black text-xl group-hover:text-indigo-600 transition-colors uppercase tracking-tighter leading-none ${isOverLimit ? 'text-rose-600' : 'text-slate-900'}`}>{c.name}</h3>
                    {isOverLimit && (
                      <span className="bg-rose-600 text-white text-[7px] font-black px-2 py-0.5 rounded-full animate-pulse tracking-widest uppercase">Over Limit</span>
                    )}
                  </div>
                ...
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl transition-all ${
                  isOverLimit ? 'bg-rose-50 text-rose-500' : 'bg-slate-50 text-slate-300 group-hover:bg-indigo-50 group-hover:text-indigo-500'
                }`}>👤</div>
              </div>
              ...
              <div className="space-y-4 relative z-10">
                <div className="flex justify-between text-[9px] font-black uppercase tracking-[0.2em]">
                  ...
                  <span className={isOverLimit ? 'text-rose-600 font-black' : (c.totalCredit >= c.creditLimit ? 'text-rose-500' : 'text-slate-600')}>
                    {( (c.totalCredit / c.creditLimit) * 100 ).toFixed(0)}%
                  </span>
                </div>
                <div className="h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner border border-slate-50">
                  <div className={`h-full rounded-full transition-all duration-1000 ${
                    isOverLimit ? 'bg-rose-600 animate-pulse' : (usagePercent > 90 ? 'bg-rose-500' : 'bg-emerald-500')
                ...
                <div className="flex justify-between items-end pt-2">
                  <div className="space-y-1">
                    ...
                    {hasDebt ? (
                      <p className={`text-2xl font-black font-mono tracking-tighter leading-none ${isOverLimit ? 'text-rose-600 underline decoration-rose-200' : 'text-slate-900'}`}>Rs. {Number(c.totalCredit).toLocaleString()}</p>
                    ) : (
                      <div className="flex items-center gap-2 mt-1">
                        <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Account Settled</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-4">
                  {hasDebt ? (
                    <button 
                      ...
                      className={`flex-[2] text-[10px] font-black py-4 rounded-2xl transition-all active:scale-95 uppercase tracking-[0.2em] shadow-lg ${
                        isOverLimit ? 'bg-rose-600 text-white hover:bg-rose-700 shadow-rose-100' : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-100'
                      }`}
                    >
                      Receive Pay
                    </button>
                  ) : (
                    <div className="flex-[2] bg-emerald-50 text-emerald-600 text-[9px] font-black py-4 rounded-2xl text-center uppercase tracking-widest border border-emerald-100 italic">
                      Zero Liability
                    </div>
                  )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-300">
            <div className="p-10 space-y-10 text-center">
              ...
              <div className={`${selectedForPayment.totalCredit > selectedForPayment.creditLimit ? 'bg-rose-50 border-rose-100' : 'bg-emerald-50 border-emerald-100'} p-8 rounded-[2.5rem] border flex flex-col items-center`}>
                <p className={`text-[9px] font-black uppercase tracking-widest mb-2 ${selectedForPayment.totalCredit > selectedForPayment.creditLimit ? 'text-rose-500' : 'text-emerald-500'}`}>Portfolio Outstanding</p>
                <p className={`text-4xl font-black font-mono tracking-tighter ${selectedForPayment.totalCredit > selectedForPayment.creditLimit ? 'text-rose-700' : 'text-emerald-700'}`}>Rs. {Number(selectedForPayment.totalCredit).toLocaleString()}</p>
              </div>
            ...
            <div className="p-10 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              ...
              <div className="flex items-center gap-10">
                 <div className="text-right">
                    ...
                    <p className={`text-3xl font-black font-mono tracking-tighter leading-none ${selectedForHistory.totalCredit > selectedForHistory.creditLimit ? 'text-rose-600 animate-pulse' : 'text-slate-900'}`}>Rs. {Number(selectedForHistory.totalCredit).toLocaleString()}</p>
                 </div>
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead className="bg-slate-950 text-slate-400 font-black uppercase tracking-widest text-[9px]">
                    <tr>
                      ...
                      <th className="px-10 py-6 text-right">Taken (+)</th>
                      <th className="px-10 py-6 text-right">Paid (-)</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-10 py-6 text-slate-500 text-[11px] font-bold uppercase leading-relaxed">
                          ...
                          <div className="flex gap-2 mt-2">
                            <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 border rounded-md ${tx.type === 'SALE' ? 'bg-indigo-50 border-indigo-100 text-indigo-500' : 'bg-emerald-50 border-emerald-100 text-emerald-500'}`}>{tx.type === 'SALE' ? 'CREDIT TAKEN' : 'CREDIT PAID'}</span>
                            <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 bg-slate-50 border border-slate-100 rounded-md text-slate-400">{tx.paymentMethod}</span>
                          </div>
                        </td>
