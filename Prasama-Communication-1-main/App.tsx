...
const App: React.FC = () => {
  ...
  const filteredTransactions = transactions.filter(t => t.branchId === activeBranch);
  const branchDaySession = filteredDaySessions.find(s => s.date === getLocalDateString());

  const branchProducts = products.map(p => ({
    ...p,
    stock: p.branchStocks && p.branchStocks[activeBranch] !== undefined ? p.branchStocks[activeBranch] : p.stock
  }));
