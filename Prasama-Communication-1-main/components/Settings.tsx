...
import { uploadAllLocalDataToFirestore } from '../services/database';
const handleUploadToFirebase = async () => {
  if (window.confirm('Upload all local data to Firebase?')) {
    try {
      await uploadAllLocalDataToFirestore();
      alert('All local data uploaded to Firebase successfully!');
    } catch (e) {
      alert('Error uploading data to Firebase. See console for details.');
      console.error(e);
    }
  }
};
...
const Settings: React.FC<SettingsProps> = ({ userProfile, setUserProfile, onExport, onImport, syncStatus }) => {
  ...
  const handleProfileSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Fix: Compute full object for the database-update setter passed from App.tsx
          ...
          <form onSubmit={handleProfileSubmit} className="space-y-6">
            <div className="flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex-1 space-y-4 w-full">
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">Corporate Contact Number</label>
                  <input name="phone" className="w-full px-5 py-3 rounded-2xl border border-slate-200 font-bold outline-none focus:border-indigo-500 transition-all" defaultValue={userProfile.phone || ""} placeholder="E.G. +94 ..." />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 bg-rose-50 rounded-3xl border border-rose-100 flex items-center gap-6 group">
              ...
              <button
                ...
                className="ml-4 bg-blue-600 text-white px-5 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-blue-700 active:scale-95 transition-all"
              >
