...
const Finance: React.FC<FinanceProps> = ({ 
  ...
  onAddRecurring, onDeleteRecurring, onUpsertAccount
   ...
   const allProtocols = useMemo(() => {
      // Collect all unique transaction types (protocols)
      const protoSet = new Set<string>();
      transactions.forEach(t => protoSet.add(t.type));
      return Array.from(protoSet);
   }, [transactions]);

   // Remove all references to sourceNodeFilter (fix crash)
   // Filter transactions by date range and protocol
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <div className="bg-white rounded-[3.5rem] shadow-sm border border-slate-100 overflow-hidden">
         <div className="p-10 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center bg-slate-50/20 gap-4 md:gap-0">
               ...
               <div className="overflow-x-auto">
                  {/* Source Node Filter removed - crash fix */}
            <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50 text-slate-400">
                              <tr>
                                    <th className="px-10 py-5 font-black uppercase tracking-widest text-[9px]">Operation / Entity</th>
                                    <th className="px-10 py-2 font-black uppercase tracking-widest text-[9px] text-center">
