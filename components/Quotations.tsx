...
const Quotations: React.FC<QuotationsProps> = ({
  ...
  const handleSaveQuotation = (status: 'DRAFT' | 'FINALIZED') => {
    ...
    const newQuotation: Quotation = {
      ...
      customerId: targetCustomer || undefined,
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .grand-total { font-size: 20px; font-weight: 800; border-bottom: 0; color: #4f46e5; }
        ...
        <body onload="window.print();">
          <div class="header">
            ...
            <div class="biz-details">
              ...
              <p style="font-size: 12px; color: #64748b; margin: 4px 0;">Date: ${new Date(q.date).toLocaleDateString()}</p>
              <p style="font-size: 12px; color: #ef4444; margin: 4px 0; font-weight: 800;">Valid Until: ${new Date(q.validUntil).toLocaleDateString()}</p>
            </div>
          </div>
                  ...
                  {quotations.map(q => (
                    <tr key={q.id} className="hover:bg-slate-50 transition-all">
                      <td className="px-8 py-5">
                        ...
                        <p className="text-[9px] text-slate-400">{new Date(q.date).toLocaleDateString()}</p>
                      </td>
                      ...
                      <td className="px-8 py-5">
                         <div className="flex flex-col gap-1">
                           ...
                           <span className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase self-start ${new Date(q.validUntil) < new Date() ? 'bg-rose-50 text-rose-500' : 'bg-emerald-50 text-emerald-500'}`}>
                             Val: {new Date(q.validUntil).toLocaleDateString()}
                           </span>
                         </div>
                      </td>
