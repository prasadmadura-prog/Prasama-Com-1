...
export interface Product {
  ...
  cost: number;
  stock: number;
  // Added branchStocks property to support multi-branch stock levels
...
export interface Transaction {
  ...
  chequeDate?: string;
  userId?: string;
  // Added branchId for tracking which shop location generated the record
  branchId?: string;
}

export interface DaySession {
  // Added id for consistency with upsert logic used in App.tsx
  id?: string;
  date: string;
  ...
  status: 'OPEN' | 'CLOSED';
  userId?: string;
  // Added branchId to support branch-specific sessions
  branchId?: string;
}
...
export interface UserProfile {
  name: string;
  branch: string;
  // Added allBranches to store available shop locations
  allBranches?: string[];
  phone?: string;
  logo?: string;
