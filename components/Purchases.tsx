...
const Purchases: React.FC<PurchasesProps> = ({
    ...
    autoSaveTimerRef.current = window.setTimeout(() => {
      ...
      onUpsertPO({
        ...
        items: poItems.map(item => {
          // If metadata is missing, try to find it now to persist it
  ...
  const vendorLedgerData = useMemo(() => {
    ...
    // Calculate Running Balance for audit integrity
    let cumulative = 0;
  ...
  const handleSavePO = async (status: POStatus = 'PENDING') => {
    ...
    // Determine status: If explicit status (like 'DRAFT') is requested, use it.
    // Otherwise, if the PO is already 'RECEIVED', keep it as 'RECEIVED'.
    // Default to 'PENDING' for new or draft commits.
    const targetStatus: POStatus = status || (selectedPO?.status === 'RECEIVED' ? 'RECEIVED' : 'PENDING');
  ...
  const printPurchaseOrder = (po: PurchaseOrder) => {
    ...
    const itemsHtml = po.items.map((it, idx) => {
      // Priority 1: Use stored product name and SKU (if available in the PO item)
      ...
      if (!displayName) {
        ...
        if (prod) {
          ...
          displaySku = displaySku || prod.sku;
        }
      }

      // Final fallbacks
      displayName = displayName || it.productId || 'Unknown Item';
      ...
      return `
        <tr>
          ...
          <td style="text-align: center; border: none; padding: 12px 14px;">${Number(it.quantity) + (Number(it.freeQuantity) || 0)}</td>
    ...
    printWindow.document.write(`
      <html>
        ...
        <body onload="window.print();">
          ...
          ${po.notes ? `
          <div style="margin-bottom: 30px; padding: 20px; background: #fffcf0; border: 1px solid #fef3c7; border-radius: 12px;">
            <h5 style="font-size: 10px; text-transform: uppercase; color: #d97706; margin: 0 0 5px 0; font-weight: 800; letter-spacing: 0.5px;">Notes & Instructions</h5>
            <p style="font-size: 12px; color: #4b5563; margin: 0; white-space: pre-wrap;">${po.notes}</p>
          </div>
          ` : ''}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">VENDORS Management System</h2>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Accounts Payable & Intake Management</p>
        </div>
        <div className="flex gap-3">
          <button onClick={onResyncBalances} className="bg-slate-100 text-slate-500 hover:text-indigo-600 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-200 transition-all active:scale-95 whitespace-nowrap">
            🔄 Reconcile
          </button>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden min-h-[500px] flex flex-col">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {/* Use paginatedPOs instead of filteredPOs */}
                {(paginatedPOs || []).map(po => (
                  <tr key={po.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="flex justify-center gap-2">
                        ...
                        <button onClick={() => { if (confirm("Delete this purchase order? This will reverse all linked stock and balance impacts.")) onDeletePO?.(po.id); }} className="p-2 border border-slate-200 rounded-lg hover:bg-rose-50 hover:text-rose-600 transition-all shadow-sm" title="Delete Order">🗑️</button>
                        <button onClick={() => printPurchaseOrder(po)} className="p-2 border border-slate-200 rounded-lg hover:bg-white transition-all shadow-sm" title="Print Order">🖨️</button>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {totalPages > 1 && (
              <div className="px-8 py-6 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between">
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                  Showing Page {currentPage} of {totalPages} ({filteredPOs.length} Records)
                </p>
        ...
        {activeTab === 'VENDORS' && (
          <div className="p-6 border-b border-slate-50 flex items-center gap-4">
            ...
            <button
              onClick={() => {
                if (confirm("Audit & Sync: Recalculate all Vendor Balances based on recorded Purchase Orders?")) {
                  onResyncBalances?.();
                }
              }}
              className="px-5 py-3 rounded-2xl border border-slate-100 bg-white hover:border-indigo-100 hover:text-indigo-600 text-slate-400 transition-all shadow-sm flex items-center gap-2 group whitespace-nowrap"
              title="Recalculate Vendor Balances"
            >
              <span className="text-sm group-hover:rotate-180 transition-transform duration-700">🔄</span>
              <span className="text-[9px] font-black uppercase tracking-widest hidden sm:inline">Sync Ledger</span>
            </button>
            {vendorSearch && (
              <button
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        onClick={() => openVendorModal(vendorLedgerData.vendor)}
                        className="w-8 h-8 rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-indigo-600 hover:border-indigo-600 transition-all flex items-center justify-center text-xs shadow-sm"
                            ...
                            <button
                              onClick={() => {
                                if (item.ledgerType === 'PO') printPurchaseOrder(item);
                                  ...
                                  const poRef = item.description?.toUpperCase().match(/PO-\d+/)?.[0];
                                  const po = purchaseOrders.find(p => p.id === poRef);
                                  if (po) printPurchaseOrder(po);
                                  else alert("Invoice manifest only available for PO-linked transactions.");
                                }
                              }}
                              className="font-mono text-[10px] font-black bg-slate-900 text-white px-3 py-1.5 rounded-lg tracking-widest uppercase hover:bg-indigo-600 transition-colors"
                            >
                              {item.id}
                            </button>
                          </td>
                          ...
                          <td className="px-4 py-5 text-center min-w-[180px]">
                            <div className="flex justify-center gap-3">
                              <button
                                onClick={() => {
                                  if (item.ledgerType === 'PO') printPurchaseOrder(item);
                                    ...
                                    const po = purchaseOrders.find(p => p.id === poRef);
                                    if (po) printPurchaseOrder(po);
                                    else alert("Detailed manifest not available for this record.");
                                  }
                                }}
                                className="w-10 h-10 flex items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-400 hover:border-emerald-600 hover:text-emerald-600 hover:bg-emerald-50 transition-all shadow-sm"
                              ...
                              <button
                                ...
                                className="w-10 h-10 flex items-center justify-center rounded-xl border border-indigo-100 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-md"
                              ...
                              <button
                                onClick={() => {
                                  if (confirm(`CRITICAL: Delete this ${item.ledgerType === 'PO' ? 'purchase order' : 'transaction'}? This will permanently reverse all linked accounting and inventory balances.`)) {
                                    if (item.ledgerType === 'PO') onDeletePO?.(item.id);
                                    else onDeleteTransaction?.(item.id);
                                  }
                                }}
                                className="w-10 h-10 flex items-center justify-center rounded-xl border border-rose-100 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all shadow-md"
              ...
              <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
                ...
                <div className="flex-1 flex flex-col p-8 overflow-y-auto custom-scrollbar">
                  <div className="grid grid-cols-3 gap-6 mb-6 shrink-0">
                    ...
                    <div className="space-y-2">
                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Main Category</label>
                      <input
                        list="po-main-cats"
                        className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold bg-white text-xs outline-none focus:border-indigo-500 uppercase"
                        placeholder="E.G. STOCK, ASSETS"
                        value={poMainCategory}
                        onChange={e => setPoMainCategory(e.target.value)}
                      />
                      <datalist id="po-main-cats">
                        <option value="OFFICE SUPPLIES" />
                        <option value="STOCK ADDITION" />
                        <option value="FIXED ASSETS" />
                        <option value="MARKETING MATERIAL" />
                      </datalist>
                    </div>
                    <div className="space-y-2">
                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest">Sub Category</label>
                      <input
                        list="po-sub-cats"
                        className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold bg-white text-xs outline-none focus:border-indigo-500 uppercase"
                        placeholder="E.G. HARDWARE, STATIONERY"
                        value={poCategory}
                        onChange={e => setPoCategory(e.target.value)}
                      />
                      <datalist id="po-sub-cats">
                  ...
                  <div className="space-y-2 mb-6 animate-in slide-in-from-top-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Notes & Remarks</label>
                    <textarea
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 font-bold bg-white text-xs outline-none focus:border-indigo-500 resize-none h-20 uppercase"
                      placeholder="ADD OPTIONAL REMARKS OR DELIVERY INSTRUCTIONS..."
                      value={poNotes}
                  ...
                  <div className="flex-1 min-h-[400px] overflow-hidden border border-slate-100 rounded-[2rem] bg-slate-50/30 flex flex-col">
                              ...
                              <tr key={idx} className="hover:bg-indigo-50/30 transition-colors group">
                                ...
                                <td className="px-6 py-3">
                                  <span className="text-[9px] font-black font-mono text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100 uppercase tracking-tighter">
                                    {product?.sku || item.productSku || 'N/A'}
                                  </span>
                                </td>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4">
                            ...
                            <p className="text-xl font-black text-emerald-700 font-mono">
                              Rs. {poItems.reduce((acc, item) => {
                                ...
                                return acc + (totalQty * Number(product.price || 0)) - (Number(item.quantity) * Number(item.cost));
                              }, 0).toLocaleString()}
                            </p>
                            <p className="text-[7px] font-bold text-emerald-600 uppercase tracking-wide mt-1">
                              AVG Margin: {totalAmount > 0 ? (((poItems.reduce((acc, item) => {
                                const product = products.find(p => p.id === item.productId);
                                const totalQty = Number(item.quantity) + (Number(item.freeQuantity) || 0);
                                return acc + (totalQty * Number(product?.price || 0));
                          ...
                          <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4">
                            ...
                            <p className="text-xl font-black text-indigo-700 font-mono">{poItems.reduce((acc, item) => acc + Number(item.quantity) + (Number(item.freeQuantity) || 0), 0)} Units</p>
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <div className="flex gap-3">
                          ...
                          <button onClick={() => handleSavePO()} className="px-8 py-3 rounded-2xl bg-slate-900 text-white font-black uppercase tracking-widest text-[9px] shadow-xl hover:bg-black transition-all active:scale-95">Commit PO</button>
                        </div>
                      </div>
                    </div>
